...
run_id=$(jq -r --arg sha "$commit" '[.workflow_runs[] | select(.head_sha == $sha and .head_branch == "main" and .event == "push" and .conclusion == "success")][0].id // empty' <<<"$runs")
...
for required in stun-e2e-linux stun-e2e-windows stun-e2e-container; do
...
echo "Verified product E2E for $commit in CI run $run_id"
